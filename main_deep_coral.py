...
parser.add_argument('--logdir', default='network_deep_coral', type=str)
...
parser.add_argument('--lambda_coral', default=0.5, type=float)
...
def coral(source, target):
    ...
    x_size = tf.shape(source)[0]
    x = tf.reshape(source, [x_size, -1])
    n = tf.cast(np.shape(x)[0],tf.float32)
    d = tf.cast(np.shape(x)[1],tf.float32)
    xm = x - tf.reduce_mean(x, 0, keep_dims=True)
    xc = tf.matmul(tf.transpose(xm), xm) / n

    x2_size = tf.shape(target)[0]
    x2 = tf.reshape(target, [x2_size, -1])
    xmt = x2 - tf.reduce_mean(x2, 0, keep_dims=True)
    xct = tf.matmul(tf.transpose(xmt), xmt) / n
...
x_target = tf.placeholder(shape=input_shape, dtype=tf.float32, name="x_target")

phase = tf.placeholder(tf.bool, name='phase')
global_step = tf.Variable(0, name='global_step', trainable=False)
out_seg, map_lambda1, map_lambda2 = architectures.ddunet(x,is_training)
out_seg_target, _, _ = architectures.ddunet(x_target, is_training)
...
coral_loss = coral(out_seg, out_seg_target)
seg_loss = 1 - Dice
l2_loss = tf.losses.get_regularization_loss()
seg_loss += l2_loss
total_loss = seg_loss + + args.lambda_coral * coral_loss
...
train_summary = []
train_summary.append(tf.summary.scalar('train/seg_loss', seg_loss))
train_summary.append(tf.summary.scalar('train/total_loss', coral_loss))
...
with tf.Session(config=config) as sess:
    if is_training:
        ...
        data_provider_test = DataGen.ImageGen(test_location, data_suffix=data_suffix, mask_suffix=mask_suffix,shuffle_data=True, n_class=1)
        ...
        try:
            for iters in range(args.train_iter):
                ...
                images_target, _, _ = data_provider_test(args.batch_size)

                if gb_step % args.train_sum_freq == 0:
                    summary_str_train = sess.run(train_summary_op, {x: images, y: labels, x_target: images_target, phase: True})
                    summary_writer_train.add_summary(summary_str_train, gb_step)
                    images, labels, _ = data_provider_valid(args.batch_size)
                    summary_str_valid = sess.run(train_summary_op, {x: images, y: labels, x_target: images_target, phase: False})
                    summary_writer_valid.add_summary(summary_str_valid, gb_step)
                else:
                    _, Dicevalue = sess.run([train_op, Dice], {x: images, y: labels, x_target: images_target, phase: True})
        ...
        print("########### Inference ############")
        saver = tf.train.Saver(tf.global_variables())
        saver.restore(sess, tf.train.latest_checkpoint(args.logdir + "/model.ckpt"))
        ...
        for i in range(0, iters, 10):
            ...
            for j in range(mask.shape[0]):
                f2 = f1_score(lab[j:j+1][0], mask[j:j+1][0], labels=None, average='micro', sample_weight=None)
                if (f2>maxDice):
                    maxDice = f2
                    print(maxDice,i,j,np.count_nonzero(lab[j:j+1][0]),np.count_nonzero(mask[j:j+1][0]))
                    maxMask = mask[j:j+1][0]
                    maxLab = lab[j:j+1][0]
                    maxInput = ip[j:j+1][0]
                    maxName = filenames[i+j]
                test_dice.append(f2)

        print('Dice is {0:0.4f}'.format(np.average(test_dice)))
        print(maxDice,maxLab.shape, np.count_nonzero(maxLab),np.size(maxLab), maxMask.shape, np.count_nonzero(maxMask),np.size(maxMask),maxName)
        fig = plt.figure()
        plt.title('Deep CORAL Output, Dice:{0:0.4f}'.format(maxDice))
        seg_out_acm=contoured_image(maxMask, maxInput)
        plt.imshow(seg_out_acm)
        plt.savefig("mmdOutput.png")
        fig = plt.figure()
        plt.title('Original Annotation')
        gt_mask = contoured_image(maxLab, maxInput)
        plt.imshow(gt_mask)
