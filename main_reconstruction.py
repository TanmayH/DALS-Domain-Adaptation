...
import DataGen
import glob
import reconstruction_architectures
...
out_seg, map_lambda1, map_lambda2 = reconstruction_architectures.ddunet(x,is_training,1)

rec_img = reconstruction_architectures.ddunet(x,is_training,0)
rec_loss = tf.nn.l2_loss(
    x / 255 - rec_img, name=None
) / (10 * 64 * 64 * 3)
...
with tf.control_dependencies(update_ops):
    target_op = tf.train.AdamOptimizer(adaptive_lr).minimize(rec_loss, global_step=global_step,
                                                            colocate_gradients_with_ops=True, name='target_op')
target_summary = []
target_summary.append(tf.summary.scalar('train/total_loss', rec_loss))

target_summary.append(tf.summary.image('Output Segmentation', rec_img))
target_summary.append(tf.summary.image('Ground Truth', x))
target_summary_op = tf.summary.merge(target_summary)
...
with tf.Session(config=config) as sess:
    if is_training:
        if restore:
            ...
            summary_writer_target = tf.summary.FileWriter(args.logdir + '/target', graph=sess.graph)
        ...
        data_provider_target = DataGen.ImageGen(target_location, data_suffix=data_suffix, mask_suffix=mask_suffix,shuffle_data=True, n_class=1)
        try:
            for iters in range(args.train_iter):
                print("Global step %d :" % (gb_step))
                gb_step = gb_step + 1
                images, labels, _ = data_provider_train(args.batch_size)
                images_t, labels_t, _ = data_provider_target(args.batch_size)
                if gb_step % args.train_sum_freq == 0:
                    ...
                    summary_str_target = sess.run(target_summary_op, {x: images_t, y: images_t, phase: True})
                    summary_writer_target.add_summary(summary_str_target, gb_step)
                    ...
                    _, rec_loss_val, r_img = sess.run([target_op, rec_loss, rec_img], {x: images_t, y: labels_t, phase: True})
                    # import pdb; pdb.set_trace()
                    print('Dice value train is {}, reconstruction loss is {}'.format(Dicevalue, rec_loss_val))
