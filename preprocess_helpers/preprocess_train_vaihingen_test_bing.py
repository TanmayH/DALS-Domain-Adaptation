...
def get_all_images_in_folder(train_folder,test_folder):
    ...
    #Vaihingen
    total_train_images = len([filename for filename in os.listdir(train_folder) \
                        if "building" in filename and "mask" not in filename])
    #Bing
    total_test_images = len([filename for filename in os.listdir(test_folder) \
                        if "building" in filename and "mask" not in filename])
    ...
    for filename in sorted(os.listdir(train_folder)):
        if "mask" not in filename:
            imgInput = cv2.imread(os.path.join(train_folder, filename))
            imgMask = cv2.imread(os.path.join(train_folder, filename[0:8]+"_mask"+filename[8:]))
    ...
    for filename in sorted(os.listdir(test_folder)):
        if "mask" not in filename:
            imgInput = cv2.imread(os.path.join(test_folder, filename))
            imgMask = cv2.imread(os.path.join(test_folder, filename[0:8]+"_mask"+filename[8:]))
            if imgInput is not None and imgMask is not None:
                # Resizing cuz Bing needs to be the same size as Vaihingen
                imgInput = cv2.resize(imgInput, (64, 64), interpolation = cv2.INTER_AREA)  
                imgMask = cv2.resize(imgMask, (64, 64), interpolation = cv2.INTER_AREA)
...
for root, dirs, files in os.walk('../network'):
    ...
    for d in dirs:
        shutil.rmtree(os.path.join(root, d))        
get_all_images_in_folder("/home.ORIG/npochhi/buildings_vaihingen/buildings/","/home.ORIG/npochhi/buildings_bing/single_buildings/")
'''
Replace the path in the above method call with path to Vaihingen_Building_Dataset/buildings/ folder, Bing_Huts_Dataset/single_buildings/ folder
'''