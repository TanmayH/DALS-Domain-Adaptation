            ...
            for filename in sorted(os.listdir(os.path.join(train_folder,directory))):
                if not("mask" in filename):
                    ...
                    if imgInput is not None and imgMask is not None:
                        ...
                        if (nonZeroCount < 0.03): #removing images that dont have lung lol
                            continue
                        imgMask = imgMask[:, :, 0]
        ...
        if "TCGA" in directory:
            for filename in sorted(os.listdir(os.path.join(test_folder,directory))):
                if not("mask" in filename):
                    ...
                    imgMask = cv2.imread(os.path.join(test_folder,directory,filename[:-4]+"_mask.tif"))
                    if imgInput is not None and imgMask is not None:
                        imgInput = cv2.resize(imgInput, (64, 64), interpolation = cv2.INTER_AREA)
                        suffixInput = "input"
                        imgMask = cv2.resize(imgMask, (64, 64), interpolation = cv2.INTER_AREA)
...
for root, dirs, files in os.walk('../network'):
    ...
    for d in dirs:
        shutil.rmtree(os.path.join(root, d))        
get_all_images_in_folder("/home.ORIG/npochhi/Lung Segmentation/", "/home.ORIG/npochhi/kaggle_3m/")
'''
Replace the path in the above method call with path to Lung_Segmentation/ folder, Lgg_Mri_Segmentation/kaggle_3m/ folder
'''