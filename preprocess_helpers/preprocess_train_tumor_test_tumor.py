...
def get_all_images_in_folder(folder):
    ...
    for directory in sorted(os.listdir(folder)):
        ...
        if "TCGA" in directory:
            for filename in sorted(os.listdir(os.path.join(folder,directory))):
                if not("mask" in filename):
                    imgInput = cv2.imread(os.path.join(folder,directory,filename))
                    imgMask = cv2.imread(os.path.join(folder,directory,filename[:-4]+"_mask.tif"))
...
for root, dirs, files in os.walk('../network'):
    ...
    for d in dirs:
        shutil.rmtree(os.path.join(root, d))        
get_all_images_in_folder("/home.ORIG/npochhi/kaggle_3m/")
'''
Replace the path in the above method call with path to Lgg_Mri_Segmentation/kaggle_3m/ folder
'''