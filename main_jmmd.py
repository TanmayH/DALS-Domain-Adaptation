...
fast_lookup = True
gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=0.2, visible_device_list='0,1')
...
def gaussian_kernel(x, y, beta):
    ...
    x_size = tf.shape(x)[0]
    y_size = tf.shape(y)[0]
    x = tf.reshape(x, [x_size, -1])
    y = tf.reshape(y, [y_size, -1])
    r = tf.expand_dims(x, 1)
    r = tf.Print(r, [tf.reduce_sum(tf.square(r - y), axis=-1)], "Tiled Diff = ", summarize=10000)
...
def Joint_MMD(x1, x2, beta=0.1):
    ...
    """
    x1x1 = gaussian_kernel(x1, x1, 0.001) 
    x1x2 = gaussian_kernel(x1, x2, 0.001)
    x2x2 = gaussian_kernel(x2, x2, 0.001)

    for i in range(-7, -3):
        x1x1 *= gaussian_kernel(x1, x1, 10**i)
        x1x2 *= gaussian_kernel(x1, x2, 10**i)
        x2x2 *= gaussian_kernel(x2, x2, 10**i)

    x1x1 = tf.Print(x1x1, [x1x1, x2x2, x1x2], "  scores ", summarize=10000)
...
seg_loss += l2_loss
mmd_score = tf.Print(mmd_score, [out_seg, out_seg_target, mmd_score, seg_loss], "MMD, Seg_Loss - ")
total_loss = seg_loss + 0.001 * mmd_score
...
with tf.Session(config=config) as sess:
    if is_training:
        ...
        fig = plt.figure()
        plt.title('MMD Output, Dice:{0:0.4f}'.format(maxDice))
