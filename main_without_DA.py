...
from sklearn.metrics import f1_score,precision_recall_fscore_support,accuracy_score,recall_score
    ...
    if is_training:
        ...
        for i in range(0, iters, 10):
            ...
            for j in range(mask.shape[0]):
                ...
                prfs = precision_recall_fscore_support(lab[j:j+1][0], mask[j:j+1][0], labels=None, average='micro', sample_weight=None)
                acc = accuracy_score(lab[j:j+1][0], mask[j:j+1][0])
                test_dice.append(f2)
                test_precision.append(prfs[0])
                test_recall.append(prfs[1])
                test_accuracy.append(acc)
        print("Avg Test Dice Score : {0:0.4f}".format(np.average(test_dice)))
        print("Avg Test Precision Score : {0:0.4f}".format(np.average(test_precision)))
        print("Avg Test Recall Score : {0:0.4f}".format(np.average(test_recall)))
        print("Avg Test Accuracy Score : {0:0.4f}".format(np.average(test_accuracy)))
        ...
        fig = plt.figure()
        plt.title('Output without Domain Adaptation, Dice:{0:0.4f}'.format(maxDice))
        seg_out_acm=contoured_image(maxMask, maxInput)
        plt.imshow(seg_out_acm)
        plt.savefig("withoutDA.png")
        fig = plt.figure()
