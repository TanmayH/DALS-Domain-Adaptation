    ...
    if is_training:
        ...
        try:
            for iters in range(args.train_iter):
                ...
                img_trg = images_target[0]
                img_trg = img_trg.transpose((2, 0, 1))


                for idx, image in enumerate(images):                   
                    image = image.transpose((2, 0, 1))
                    image = FDA_source_to_target_np( image, img_trg, L=0.01 )
                    images[idx] = image.transpose((1, 2, 0))
        ...
        fig = plt.figure()
        plt.title('FDA Output, Dice:{0:0.4f}'.format(maxDice))
